...
为了更好地理解"暂停并澄清提示词"模板的使用方法,我们将它应用到用不同AI工具实现"高考志愿填报建议"这个实际案例中。

## 4. 使用 DeepSeek 官网聊天应用实践"暂停并澄清提示词"

如果你使用AI主要处理公开且非敏感的信息,可以直接在DeepSeek官网聊天应用中使用"暂停并澄清提示词"。这样能让AI在生成内容前主动暂停,与你澄清需求,并推荐多个方案供选择。通过这种方式,AI只需生成一次内容,就能让质量达到95分。

### 4.1 准备"暂停并澄清"风格提示词

下面是根据第3节介绍的模板，将"开场提示词（个性化）"、"开场提示词（通用）"、"文本知识库"和"RIPER-5系统提示词"合并而成的完整提示词。注意：由于DeepSeek官网聊天应用无法同时支持"搜索"功能和上传文本知识库，因此"高考志愿填报建议"所需的3个PDF文件已通过复制粘贴转为纯文本内容，并以文本知识库的方式嵌入到提示词中。这样就能在DeepSeek官网聊天应用中打开“搜索”功能。
...
### 4.2 体验"暂停并澄清"风格提示词的暂停并澄清特性

在电脑上用浏览器打开DeepSeek官网聊天应用链接https://chat.deepseek.com/,打开“DeepThink（深度思考）”和“Search（搜索）”选项，然后将4.1节中的完整的提示词复制并粘贴到提示词输入区，再点击发送按钮，即可开启对话。

此后按照 RESEARCH -> INNOVATE -> PLAN -> EXECUTE -> REVIEW 的顺序,按4.1节中提示词中的要求依次输入指令（如“ENTER INNOVATE MODE”、“ENTER PLAN MODE”、“ENTER EXECUTE MODE”、“ENTER REVIEW MODE”）,即可体验暂停并澄清特性。

在RESEARCH模式下需要注意：即使AI声称已完全理解需求，甚至主动向你澄清疑问并获得回复，你仍要追问一句"你是否还有关于我的需求需要澄清的问题？我可以回答。"不要急于进入下一模式。即便AI已自行进入INNOVATE模式，你也要在提示词中要求它回到RESEARCH模式，坚持追问，直到AI明确表示没有更多问题需要澄清为止。

若EXECUTE模式生成的内容符合预期，可跳过REVIEW模式。

## 5. 使用 Dify 实践"暂停并澄清提示词"

如果需要使用企业私有化部署的大模型来处理不便上传外网的内部文档,可以使用 Dify 工具实践"暂停并澄清提示词"。Dify 是一个开源工具,企业常用它连接私有部署的大型模型,构建带有知识库的智能体。这样既能让 AI 更精确地检索企业内部信息,又能避免使用外部云端模型导致的文档泄露风险。
...
### 5.4 使用 Dify 创建并配置"大学专业选择顾问"智能体
...
### 5.7 在 Dify 智能体中配置 Conversation Opener
...
本节样例智能体在Dify云服务上的链接为: https://udify.app/chat/cj9jnvnAaHx7H0S8

### 5.9 体验"高考志愿专业填报建议"智能体的暂停并澄清特性

用浏览器打开上述链接,点击左上方"Start new chat"按钮。复制开场白结尾处外层【】内的内容,粘贴到下面的提示词输入区。将内层【】中的内容替换为适合自身情况的信息,然后点击右下角的提交按钮发送给AI,即可开启对话。如下所示：
...
此后按照 RESEARCH -> INNOVATE -> PLAN -> EXECUTE -> REVIEW 的顺序,按上述要求依次输入指令（如“ENTER INNOVATE MODE”、“ENTER PLAN MODE”、“ENTER EXECUTE MODE”、“ENTER REVIEW MODE”）,即可体验暂停并澄清特性。
